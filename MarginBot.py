...
def random_chance(reverse_chance, win, loss):
...
def save_as(filedir, new=False):
    if new:
        with open(filedir, 'w+') as File:
            structure = dict(settings=None, eventlog=None)
            json.load(File, structure)
    with open('Settings.json', 'r') as SettingsFile:
        settings = json.load(SettingsFile)
        setjson(filedir, 'settings', settings)
    with open('EventLog.json', 'r') as EventLogFile:
        event_log = json.load(EventLogFile)
        setjson(filedir, 'eventlog', event_log)


def reload(source_file_path):
    with open('Settings.json', 'w') as SettingsFile:
        json.dump(getjson(source_file_path, 'settings'), SettingsFile)
    with open('EventLog.json', 'w') as EventLogFile:
        json.dump(getjson(source_file_path, 'eventlog'), EventLogFile)


@bot.message_handler(commands=['back'])
def back(message):
    reload('PrevState.json')
    bot.send_message(message.chat.id, 'Возвращаюсь к предыдущему шагу')


@bot.message_handler(commands=['resetlog'])
def handle_resetlog(message):
    save_as('PrevState.json')
    resetlog()
...
def processmarket(market, pos_in_roundstate, message):
            ...
            for team in current_game_team_list:
                for active in team['actives']:
                    if active['current_actmarket'] == pos_in_roundstate:
                        exec(num['vname'] + '+= 1')
                        break
        # ВЫЧИСЛЕНИЕ КОНСТАНТЫ, ЕСЛИ ЭТО КОЛИЧЕСТВО АКТИВОВ В ЭТОМ РЫНКЕ
        if num['type'] == 'actives_in_market':
            exec(num['vname'] + '= 0')
            for team in current_game_team_list:
                for active in team['actives']:
                    if active['current_actmarket'] == pos_in_roundstate:
                        exec(num['vname'] + '+= 1')
        # ВЫЧИСЛЕНИЕ КОНСТАНТЫ, ЕСЛИ ЭТО
    # ВЫЧИСЛЕНИЕ КОЭФФИЦИЕНТОВ
    coef = eval(market['coef_evaluation'])
    bot.send_message(message.chat.id, 'Коэффициент для рынка '
                     + str(pos_in_roundstate + 1)
                     + ' равен '
                     + str(coef))
    print(coef)
...
def processroundstart(message):
    ...
    if round['type'] == 'quiz':
        ...
        bot.send_message(message.chat.id, 'Начинаю раунд ' + str(getlog('current_game_round_num') + 1) + ', в этом '
                                                                                                         ...
                                                                                                         'Чтобы закончить раунд, введите /nextround')
    if round['type'] == 'invest':
        modlog('pending_nextround', True)
        ...
        bot.send_message(message.chat.id, 'Начинаю раунд '
                         ...
                         + ', сейчас нужно будет сначала ввести / + номер команды в списке команд, а затем по очереди '
                           'ввести / + номера рынков из раунда, в которые пойдут активы выбранной команды.\n'
                           'Для вывода списка активных команд введите /showteamlist\n'
...
def startgame(message):
    ...
    processroundstart(message)


@bot.message_handler(commands=['nextround'])
def nextround(message):
    save_as('PrevState.json')
    if getlog('invest_gameplay') or getlog('invest_gameplay_acts'):
        markets = getlog('current_game_markets_list')
        for i, market in enumerate(markets):
            processmarket(market, i, message)
    ...
    if getlog('pending_nextround'):
        ...
        modlog('current_game_team_num', 0)
        modlog('current_game_act_num', 0)
        modlog('quiz_gameplay', False)
        modlog('quiz_gameplay_coef', False)
        modlog('invest_gameplay', False)
        modlog('invest_gameplay_acts', False)
        modlog('current_game_team_list', getsettings('teams'))
...
def finish(message):
    save_as('PrevState.json')
    modsettings('teams', getlog('current_game_team_list'))
    bot.send_message(message.chat.id, 'ИГРА ОКОНЧЕНА, ЭТО PLACEHOLDER-СООБЩЕНИЕ. КОМАНДА /showteamlist ВСЁ ЕЩЁ РАБОТАЕТ.\nДл начала новой игры с указанными настройками введите'
...
def nummessageresponce(message):
    ...
    # ОБРАБОТКА ЧИСЛА ПРИ ВВОДЕ КОЭФФИЦИЕНТА ДЛЯ КВИЗА
    elif getlog('quiz_gameplay_coef'):
        modlog('quiz_gameplay_coef')
        modlog('quiz_gameplay', True)
        teams = getsettings('teams')
        acts = teams[getlog('current_gameplay_team_num')]['actives']
        for i, act in enumerate(acts):
            acts[i]['actvalue'] *= float(message.text)
        teams[getlog('current_gameplay_team_num')]['actives'] = acts
        modsettings('teams', teams)
        bot.send_message(message.chat.id, 'Коэффициент введён\n')
        bot.send_message(message.chat.id, 'Чтобы умножить активы команды на какое-либо число, введите команду с её номером (например /1)\n'
                                          'Для вывода списка команд и активов введите /showteamlist\n'
                                          'Для перехода к следующему раунду введите /nextround')
...
def slashint(message):
    ...
    # ВЫБОР РЫНКА ПРИ НАСТРОЙКЕ С НУЛЯ
    ...
    elif getlog('quiz_gameplay'):
        modlog('quiz_gameplay')
        modlog('quiz_gameplay_coef', True)
        modlog('current_gameplay_team_num', value - 1)
        bot.send_message(message.chat.id, 'Выбрана команда '
                         + str(value)
                         + ', на какой коэффициент нужно будет умножить её активы?')


    elif getlog('invest_gameplay'):
        modlog('invest_gameplay')
        modlog('invest_gameplay_acts', True)
        modlog('current_game_team_num', value - 1)
        modlog('current_game_act_num', 0)
        bot.send_message(message.chat.id, 'Выбрана команда ' +
                         getlog('current_game_team_list')[value - 1]['teamname'])
        bot.send_message(message.chat.id, 'Введите номер рынка, куда пойдёт актив '
                         + str(getlog('current_game_act_num') + 1)
                         + ' (для вывода в чат списка раундов введите команду /showroundmarkets )')


    elif getlog('invest_gameplay_acts'):
        ...
        if getlog('current_game_act_num') < getsettings('total_actives_num'):
            bot.send_message(message.chat.id, 'Рынок выбран, перехожу к следующему активу')
            bot.send_message(message.chat.id, 'Введите номер рынка, куда пойдёт актив '
                             + str(getlog('current_game_act_num') + 1)
                             + ' (для вывода в чат списка раундов введите команду /showroundmarkets )')
        else:
            modlog('current_game_act_num', 0)
            modlog('invest_gameplay_acts')
            modlog('invest_gameplay', True)
            bot.send_message(message.chat.id, 'Все активы команды настроены.\n'
                                              'Введите номер в списке следующей команды для указания её рынков\n'
                                              'Для вывода списка команд введите /showteamlist\n'
                                              'Для подсчёта коэффициентов команд и перехода к следующему раунду введите /nextround')
    ...
    if getlog('is_fullsetup_teamname'):
        ...
        if getlog('current_team_setup_num') >= getsettings('total_team_num'):
            ...
            bot.send_message(message.chat.id, 'Для настройки бота введите /setup \n'
                                              'Для сброса настроек бота введите /reset (если что-то идёт не так, '
                                              'лучше жать сюда)\n '
                                              'Для начала игры с выбранными настройками введите /startgame\n '
                                              'Для вывода в чат списка команд и их активов введите /showteamlist', )
            return
